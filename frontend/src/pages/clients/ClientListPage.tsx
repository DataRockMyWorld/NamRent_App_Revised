...
import { Plus, Building2, CheckCircle, Clock, AlertTriangle, Search, X, ChevronDown } from "lucide-react";
import { apiClient } from "@/services/apiClient";
import type { Client, PaginatedResponse } from "@/types";
import { Badge, kycStatusVariant, ListPageSkeleton, RowActions } from "@/components/ui";
...
function paginationBtn(disabled: boolean): React.CSSProperties {
  return { height: 34, paddingInline: 14, borderRadius: 8, background: disabled ? "var(--color-bg-subtle)" : "#fff", border: "1px solid var(--color-border)", color: disabled ? "var(--color-text-disabled)" : "var(--color-text-primary)", fontSize: 13, fontWeight: 500, fontFamily: "inherit", cursor: disabled ? "not-allowed" : "pointer", opacity: disabled ? 0.6 : 1 };
}

// ─── Row ──────────────────────────────────────────────────────────────────────
function ClientRow({ c, isLast }: { c: Client; isLast: boolean }) {
  ...
  return (
    <tr style={{ borderBottom: isLast ? "none" : "1px solid var(--color-border)", background: hovered ? "var(--color-bg-hover)" : "transparent", transition: "background 0.1s" }} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
      <td style={tdStyle}>
        <Link to={`/clients/${c.id}`} style={{ fontSize: 13, fontWeight: 600, color: "var(--color-primary)", textDecoration: "none" }} className="hover:underline">{c.company_name}</Link>
        {c.contact_person_name && <p style={{ fontSize: 12, color: "var(--color-text-muted)", marginTop: 2 }}>{c.contact_person_name}</p>}
      </td>
      <td style={tdStyle}><span style={{ fontSize: 13, color: "var(--color-text-muted)" }}>{labelify(c.client_type)}</span></td>
      <td style={tdStyle}><span style={{ fontSize: 13, color: "var(--color-text-primary)" }}>{c.email}</span></td>
      <td style={tdStyle}><span style={{ fontSize: 13, color: "var(--color-text-muted)" }}>{c.phone || "—"}</span></td>
      <td style={tdStyle}><Badge variant={kycStatusVariant(c.kyc_status)}>{labelify(c.kyc_status)}</Badge></td>
      <td style={tdStyle}>
      ...
      <td style={{ ...tdStyle, paddingInline: 8 }}>
        <RowActions actions={[{ label: "View", to: `/clients/${c.id}` }, { label: "Edit", to: `/clients/${c.id}/edit` }]} />
      </td>
    </tr>
  );
}
  ...
  return (
    <div style={{ padding: "80px 32px", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" }}>
      <div style={{ width: 56, height: 56, borderRadius: "50%", background: "#EBF5FF", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 20 }}>
        <Building2 size={24} style={{ color: "#3B96E8" }} />
      </div>
      <p style={{ fontSize: 16, fontWeight: 600, color: "var(--color-text-heading)", marginBottom: 10 }}>
        {isFiltered ? "No matching clients" : "No clients added yet"}
      </p>
      <p style={{ fontSize: 14, color: "var(--color-text-muted)", lineHeight: 1.65, maxWidth: 400, marginBottom: !isFiltered ? 28 : 0 }}>
        {isFiltered ? "Try adjusting your search to find what you're looking for." : "Add your first client to start managing fleet assignments and contracts."}
      </p>
      {!isFiltered && (
...
export default function ClientListPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => apiClient.get<PaginatedResponse<Client>>("/clients/", { params: { page, search: search || undefined, page_size: PAGE_SIZE } }).then(r => r.data),
  });
  const activeQ    = useQuery({ queryKey: ["clients-stat", "active"],    queryFn: () => apiClient.get<PaginatedResponse<Client>>("/clients/", { params: { account_status: "ACTIVE",    page_size: 1 } }).then(r => r.data.count) });
  const pendingQ   = useQuery({ queryKey: ["clients-stat", "pending"],   queryFn: () => apiClient.get<PaginatedResponse<Client>>("/clients/", { params: { kyc_status: "PENDING",       page_size: 1 } }).then(r => r.data.count) });
  const suspendedQ = useQuery({ queryKey: ["clients-stat", "suspended"], queryFn: () => apiClient.get<PaginatedResponse<Client>>("/clients/", { params: { account_status: "SUSPENDED", page_size: 1 } }).then(r => r.data.count) });
  ...
  return (
    <div className="page-container" style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
        <div>
          <h1 style={{ fontSize: 27, fontWeight: 600, color: "var(--color-text-heading)", letterSpacing: "-0.022em", lineHeight: 1.2 }}>Clients</h1>
          <p style={{ fontSize: 14, color: "var(--color-text-muted)", marginTop: 5, lineHeight: 1.5 }}>Manage client companies, onboarding status, and account access.</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <ListStatCard icon={Building2}     label="Total Clients"  value={totalCount}           sub={totalCount === 0 ? "No clients registered" : "registered companies"}           accentColor="#3B96E8" iconBg="#EBF5FF" iconColor="#3B96E8" />
        <ListStatCard icon={CheckCircle}   label="Active"         value={activeQ.data ?? 0}    sub={(activeQ.data ?? 0) > 0 ? "accounts in good standing" : "No active accounts"}    accentColor="#12B76A" iconBg="#ECFDF3" iconColor="#12B76A" />
        <ListStatCard icon={Clock}         label="Pending KYC"    value={pendingQ.data ?? 0}   sub={(pendingQ.data ?? 0) > 0 ? "awaiting verification" : "All KYC verified"}         accentColor="#F79009" iconBg="#FFFAEB" iconColor="#F79009" />
      ...
      <div style={{ background: "#fff", border: "1px solid var(--color-border)", borderRadius: 14, boxShadow: "var(--shadow-card)", padding: "14px 18px" }}>
        <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 10 }}>
          <div style={{ position: "relative", flex: "1 1 200px", maxWidth: 300 }}>
            ...
            <input type="text" value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} placeholder="Search by name, email, contact…" style={{ width: "100%", height: 38, paddingInline: "36px 32px", borderRadius: 10, border: "1px solid var(--color-border)", background: search ? "#fff" : "var(--color-bg-subtle)", fontSize: 13, fontFamily: "inherit", color: "var(--color-text-primary)", outline: "none", transition: "border-color 0.15s, background 0.15s" }} onFocus={e => { e.currentTarget.style.borderColor = "#3B96E8"; e.currentTarget.style.background = "#fff"; }} onBlur={e => { e.currentTarget.style.borderColor = "var(--color-border)"; if (!search) e.currentTarget.style.background = "var(--color-bg-subtle)"; }} />
        ...
        {clients.length === 0 ? <EmptyCard isFiltered={hasActiveFilters} /> : (
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>
                {clients.map((c, idx) => <ClientRow key={c.id} c={c} isLast={idx === clients.length - 1} />)}
              </tbody>
            </table>
          </div>
        )}
