...
import { Plus, ArrowLeftRight, Send, Clock, CheckCircle, Search, X } from "lucide-react";
...
function tradeInStatusVariant(status: string) {
  if (["ACTIVE", "CONTRACTED", "APPROVED", "VALUATION_ACCEPTED"].includes(status)) return "success" as const;
  if (["UNDER_REVIEW", "VALUATION_REQUESTED", "VALUATION_SUBMITTED", "OFFER_RECEIVED"].includes(status)) return "warning" as const;
  if (["SUBMITTED", "REPLACEMENT_REQUESTED"].includes(status)) return "info" as const;
...
function TradeInRow({ r, isLast }: { r: TradeInRequest; isLast: boolean }) {
  ...
  return (
    <tr style={{ borderBottom: isLast ? "none" : "1px solid var(--color-border)", background: hovered ? "var(--color-bg-hover)" : "transparent", transition: "background 0.1s" }} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
      <td style={tdStyle}>
        <Link to={`/tradeins/${r.id}`} style={{ fontSize: 13, fontWeight: 600, color: "var(--color-primary)", textDecoration: "none" }} className="hover:underline">{r.reference_number}</Link>
      </td>
      <td style={tdStyle}><span style={{ fontSize: 13, color: "var(--color-text-primary)" }}>{r.client_name}</span></td>
      <td style={tdStyle}><span style={{ fontSize: 13, color: "var(--color-text-muted)" }}>{r.vehicle_display}</span></td>
      <td style={tdStyle}><span style={{ fontSize: 13, color: "var(--color-text-muted)" }}>{labelify(r.trade_in_condition)}</span></td>
      <td style={tdStyle}><Badge variant={tradeInStatusVariant(r.status)}>{labelify(r.status)}</Badge></td>
      ...
      <td style={{ ...tdStyle, paddingInline: 8 }}>
        <RowActions actions={[{ label: "View", to: `/tradeins/${r.id}` }]} />
      </td>
    </tr>
  );
}
  ...
  return (
    <div style={{ padding: "80px 32px", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" }}>
      <div style={{ width: 56, height: 56, borderRadius: "50%", background: "#EBF5FF", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 20 }}>
        <ArrowLeftRight size={24} style={{ color: "#3B96E8" }} />
      </div>
      <p style={{ fontSize: 16, fontWeight: 600, color: "var(--color-text-heading)", marginBottom: 10 }}>{isFiltered ? "No matching trade-ins" : "No trade-in requests yet"}</p>
      <p style={{ fontSize: 14, color: "var(--color-text-muted)", lineHeight: 1.65, maxWidth: 400, marginBottom: (!isFiltered && isClient) ? 28 : 0 }}>
        {isFiltered ? "Try adjusting your search to find what you're looking for." : "Submit a trade-in request to exchange existing fleet vehicles."}
      </p>
      {!isFiltered && isClient && (
        <Link to="/tradeins/new" style={{ textDecoration: "none" }}>
...
export default function TradeInListPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => apiClient.get<PaginatedResponse<TradeInRequest>>("/tradeins/", { params: { page, search: search || undefined, page_size: PAGE_SIZE } }).then(r => r.data),
  });
  const submittedQ   = useQuery({ queryKey: ["tradeins-stat", "submitted"],   queryFn: () => apiClient.get<PaginatedResponse<TradeInRequest>>("/tradeins/", { params: { status: "SUBMITTED",   page_size: 1 } }).then(r => r.data.count) });
  const underReviewQ = useQuery({ queryKey: ["tradeins-stat", "underreview"], queryFn: () => apiClient.get<PaginatedResponse<TradeInRequest>>("/tradeins/", { params: { status: "UNDER_REVIEW", page_size: 1 } }).then(r => r.data.count) });
  const activeQ      = useQuery({ queryKey: ["tradeins-stat", "active"],      queryFn: () => apiClient.get<PaginatedResponse<TradeInRequest>>("/tradeins/", { params: { status: "ACTIVE",       page_size: 1 } }).then(r => r.data.count) });
  ...
  return (
    <div className="page-container" style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
        <div>
          <h1 style={{ fontSize: 27, fontWeight: 600, color: "var(--color-text-heading)", letterSpacing: "-0.022em", lineHeight: 1.2 }}>Trade-ins</h1>
          <p style={{ fontSize: 14, color: "var(--color-text-muted)", marginTop: 5, lineHeight: 1.5 }}>Manage fleet vehicle trade-in requests, valuations, and replacements.</p>
        </div>
        {isClient && (
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <ListStatCard icon={ArrowLeftRight} label="Total Requests" value={totalCount}                sub={totalCount === 0 ? "No trade-ins submitted" : "in trade-in pipeline"}            accentColor="#3B96E8" iconBg="#EBF5FF" iconColor="#3B96E8" />
        ...
        <ListStatCard icon={Clock}          label="Under Review"   value={underReviewQ.data ?? 0}    sub={(underReviewQ.data ?? 0) > 0 ? "being evaluated" : "None under review"}           accentColor="#F79009" iconBg="#FFFAEB" iconColor="#F79009" />
        <ListStatCard icon={CheckCircle}    label="Active"         value={activeQ.data ?? 0}         sub={(activeQ.data ?? 0) > 0 ? "contracted & active" : "No active trade-ins"}          accentColor="#12B76A" iconBg="#ECFDF3" iconColor="#12B76A" />
      </div>
      ...
      <div style={{ background: "#fff", border: "1px solid var(--color-border)", borderRadius: 14, boxShadow: "var(--shadow-card)", padding: "14px 18px" }}>
        <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 10 }}>
          <div style={{ position: "relative", flex: "1 1 200px", maxWidth: 300 }}>
            ...
            <input type="text" value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} placeholder="Search reference, client, vehicle…" style={{ width: "100%", height: 38, paddingInline: "36px 32px", borderRadius: 10, border: "1px solid var(--color-border)", background: search ? "#fff" : "var(--color-bg-subtle)", fontSize: 13, fontFamily: "inherit", color: "var(--color-text-primary)", outline: "none", transition: "border-color 0.15s, background 0.15s" }} onFocus={e => { e.currentTarget.style.borderColor = "#3B96E8"; e.currentTarget.style.background = "#fff"; }} onBlur={e => { e.currentTarget.style.borderColor = "var(--color-border)"; if (!search) e.currentTarget.style.background = "var(--color-bg-subtle)"; }} />
        ...
        {requests.length === 0 ? <EmptyCard isFiltered={hasActiveFilters} isClient={isClient} /> : (
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>{requests.map((r, i) => <TradeInRow key={r.id} r={r} isLast={i === requests.length - 1} />)}</tbody>
            </table>
          </div>
        )}
      </div>
