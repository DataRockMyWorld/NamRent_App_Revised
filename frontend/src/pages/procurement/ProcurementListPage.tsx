...
import { Plus, ShoppingCart, Inbox, CheckCircle2, CheckCircle, Search, X } from "lucide-react";
...
function ProcurementRow({ r, isLast }: { r: ProcurementRequest; isLast: boolean }) {
  ...
  return (
    <tr style={{ borderBottom: isLast ? "none" : "1px solid var(--color-border)", background: hovered ? "var(--color-bg-hover)" : "transparent", transition: "background 0.1s" }} onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}>
      <td style={tdStyle}>
        <Link to={`/procurement/${r.id}`} style={{ fontSize: 13, fontWeight: 600, color: "var(--color-primary)", textDecoration: "none" }} className="hover:underline">{r.reference_number}</Link>
      </td>
      <td style={tdStyle}><span style={{ fontSize: 13, color: "var(--color-text-primary)" }}>{r.client_name}</span></td>
      <td style={tdStyle}><span style={{ fontSize: 13, color: "var(--color-text-muted)" }}>{r.vehicle_type}</span></td>
      <td style={tdStyle}><span style={{ fontSize: 13, color: "var(--color-text-primary)", fontVariantNumeric: "tabular-nums" }}>{r.quantity}</span></td>
      <td style={tdStyle}><span style={{ fontSize: 13, color: "var(--color-text-muted)" }}>{labelify(r.arrangement_type)}</span></td>
      <td style={tdStyle}><span style={{ fontSize: 13, color: "var(--color-text-muted)", fontVariantNumeric: "tabular-nums" }}>{r.offer_count ?? 0}</span></td>
      <td style={tdStyle}><Badge variant={procurementStatusVariant(r.status)}>{labelify(r.status)}</Badge></td>
      ...
      <td style={{ ...tdStyle, paddingInline: 8 }}>
        <RowActions actions={[{ label: "View", to: `/procurement/${r.id}` }]} />
      </td>
    </tr>
  );
}
  ...
  return (
    <div style={{ padding: "80px 32px", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" }}>
      <div style={{ width: 56, height: 56, borderRadius: "50%", background: "#EBF5FF", display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 20 }}>
        <ShoppingCart size={24} style={{ color: "#3B96E8" }} />
      </div>
      <p style={{ fontSize: 16, fontWeight: 600, color: "var(--color-text-heading)", marginBottom: 10 }}>{isFiltered ? "No matching requests" : "No procurement requests yet"}</p>
      <p style={{ fontSize: 14, color: "var(--color-text-muted)", lineHeight: 1.65, maxWidth: 400, marginBottom: (!isFiltered && isClient) ? 28 : 0 }}>
        {isFiltered ? "Try adjusting your search to find what you're looking for." : "Submit a procurement request to source vehicles for your fleet."}
      </p>
      {!isFiltered && isClient && (
        <Link to="/procurement/new" style={{ textDecoration: "none" }}>
...
export default function ProcurementListPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => apiClient.get<PaginatedResponse<ProcurementRequest>>("/procurement/", { params: { page, search: search || undefined, page_size: PAGE_SIZE } }).then(r => r.data),
  });
  const offersQ    = useQuery({ queryKey: ["procurement-stat", "offers"],   queryFn: () => apiClient.get<PaginatedResponse<ProcurementRequest>>("/procurement/", { params: { status: "OFFERS_RECEIVED", page_size: 1 } }).then(r => r.data.count) });
  const selectedQ  = useQuery({ queryKey: ["procurement-stat", "selected"], queryFn: () => apiClient.get<PaginatedResponse<ProcurementRequest>>("/procurement/", { params: { status: "OFFER_SELECTED",  page_size: 1 } }).then(r => r.data.count) });
  const activeQ    = useQuery({ queryKey: ["procurement-stat", "active"],   queryFn: () => apiClient.get<PaginatedResponse<ProcurementRequest>>("/procurement/", { params: { status: "ACTIVE",          page_size: 1 } }).then(r => r.data.count) });
  ...
  return (
    <div className="page-container" style={{ display: "flex", flexDirection: "column", gap: 24 }}>
      ...
      <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16, flexWrap: "wrap" }}>
        <div>
          <h1 style={{ fontSize: 27, fontWeight: 600, color: "var(--color-text-heading)", letterSpacing: "-0.022em", lineHeight: 1.2 }}>Procurement</h1>
          <p style={{ fontSize: 14, color: "var(--color-text-muted)", marginTop: 5, lineHeight: 1.5 }}>Manage vehicle sourcing requests, dealer offers, and procurement status.</p>
        </div>
        {isClient && (
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <ListStatCard icon={ShoppingCart}  label="Total Requests"  value={totalCount}              sub={totalCount === 0 ? "No requests submitted" : "in procurement pipeline"}          accentColor="#3B96E8" iconBg="#EBF5FF" iconColor="#3B96E8" />
        <ListStatCard icon={Inbox}         label="Offers Received" value={offersQ.data ?? 0}        sub={(offersQ.data ?? 0) > 0 ? "awaiting selection" : "No offers pending"}             accentColor="#F79009" iconBg="#FFFAEB" iconColor="#F79009" />
        <ListStatCard icon={CheckCircle2}  label="Offer Selected"  value={selectedQ.data ?? 0}      sub={(selectedQ.data ?? 0) > 0 ? "pending contract" : "No offers selected"}            accentColor="#6366F1" iconBg="#EEF2FF" iconColor="#6366F1" />
        <ListStatCard icon={CheckCircle}   label="Active"          value={activeQ.data ?? 0}        sub={(activeQ.data ?? 0) > 0 ? "contracted & active" : "No active procurements"}       accentColor="#12B76A" iconBg="#ECFDF3" iconColor="#12B76A" />
      </div>
      ...
      <div style={{ background: "#fff", border: "1px solid var(--color-border)", borderRadius: 14, boxShadow: "var(--shadow-card)", padding: "14px 18px" }}>
        <div style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 10 }}>
          <div style={{ position: "relative", flex: "1 1 200px", maxWidth: 300 }}>
            ...
            <input type="text" value={search} onChange={e => { setSearch(e.target.value); setPage(1); }} placeholder="Search reference, client, vehicle type…" style={{ width: "100%", height: 38, paddingInline: "36px 32px", borderRadius: 10, border: "1px solid var(--color-border)", background: search ? "#fff" : "var(--color-bg-subtle)", fontSize: 13, fontFamily: "inherit", color: "var(--color-text-primary)", outline: "none", transition: "border-color 0.15s, background 0.15s" }} onFocus={e => { e.currentTarget.style.borderColor = "#3B96E8"; e.currentTarget.style.background = "#fff"; }} onBlur={e => { e.currentTarget.style.borderColor = "var(--color-border)"; if (!search) e.currentTarget.style.background = "var(--color-bg-subtle)"; }} />
        ...
        {requests.length === 0 ? <EmptyCard isFiltered={hasActiveFilters} isClient={isClient} /> : (
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              ...
              <tbody>{requests.map((r, i) => <ProcurementRow key={r.id} r={r} isLast={i === requests.length - 1} />)}</tbody>
            </table>
          </div>
        )}
      </div>
